...
## ✅ СТАТУС: виконано 2026-09-21 (пауза з 2026-09-18 знята того ж дня)
...
- Хештег береться з `HUB_MEMBERS.primary`, а не з `articleSection`: секція заповнена менш ніж у половині постів і суперечить
...
**Зроблено — Фаза 2 (розсилка), 2026-09-21:** `npm run announce -- <розділ/slug>` робить усі три канали.
- Отримувачі — `confirmed` підписники розділу (або ті, хто вибрав усе); `announcement_deliveries` тримає стан
  **по кожному отримувачу**: падіння на 50-му листі не позначає розсилку виконаною.
- Послідовність на кожного: claim рядка `sending` **до** виклику Resend → send із детермінованим
  `Idempotency-Key` → запис `sent`/`failed`. Повторний прогін бере лише тих, кому ще не надіслано.
- Ліміти: 90 листів за прогін (безкоштовний Resend — 100/добу) і пауза 600 мс (≈2 запити/с). Решта
  переноситься на наступний прогін і логується, а не зникає.
- Кожен лист має видиме посилання на відписку й заголовки `List-Unsubscribe` + `List-Unsubscribe-Post`.
- Самолікування: рядок із випадковим хешем відписки (створений до детермінованого токена) виправляється
  перед надсиланням — інакше та людина не змогла б відписатися.
- Тести: `tests/announce-email.test.js` (13) + `tests/email.test.js` (8). Мутації: 12 з 12 убито.

**Живий тест (21.09.2026) — пройдено повністю.** Підписка через форму на `/creative/` → лист із підтвердженням
у **вхідних** (не в спамі, тобто DKIM/SPF/DMARC зійшлися) → підтвердження за 60 с → анонс поста про пісню
«Ми ж тільки на каву!» у канал і листом → **кнопка «Відписатися» в інтерфейсі Gmail** (заголовок
`List-Unsubscribe`, POST без `Origin` — саме те, заради чого на цьому ендпоінті свідомо немає CSRF-перевірки)
→ повторна підписка. Три натискання «підписатися» поспіль дали **один** лист: ліміт на адресу тримає.
У `announcement_deliveries` доставка записана як `sent` із `resend_id`.

**По ходу довелося полагодити двічі:** зникло безкоштовне сховище Upstash KV (обидві форми віддавали 503 —
fail-closed; коментарі не приймалися з 11 серпня), і після зміни пароля бази у Vercel лишився старий
`DATABASE_URL` (функції висли, 504). Обидва випадки — в «Технічних нотатках» AGENTS.md як те, що треба перевіряти.

**Не зроблено:** підняття `_dmarc.send` до `reject` після прогрівання; черга-outbox, яка прибрала б часовий
оракул на `/api/subscribe` (відоме обмеження); моніторинг живості форм (щоб наступний такий збій не жив місяць).
