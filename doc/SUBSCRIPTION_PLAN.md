...
**Не зроблено:** живий тест розсилки (потрібні `DATABASE_URL`, `RESEND_API_KEY`, `EMAIL_HASH_SECRET` у
локальному `.env`); підняття `_dmarc.send` до `reject` після прогрівання; черга-outbox, яка прибрала б
часовий оракул на `/api/subscribe` (відоме обмеження).
