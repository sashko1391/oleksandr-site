...
## Переробка в особистий сайт (2026-09-18)

**Напрям:** parkinsandr.tech стає особистим сайтом автора і розробника; розробка на замовлення лишається окремим
комерційним хабом `/services/` (активний дохід). Головний план — `doc/PERSONAL_SITE_PLAN.md`, Ф1 — `doc/SERVICES_HUB_PLAN.md`.

**Нова IA (хаби поверх наявних URL, пости не переносяться):** `/` (особиста головна, Ф3) · `/journal/` (щоденник, есеї,
подорожі, пам'ять) · `/code/` · `/creative/` · `/parkinson/` (Ф2) · `/services/` (Ф1) · `/pro-mene/` · пізніше `/support/`, `/shop/`.

**Ключові архітектурні рішення:**
- Static зараз, Astro — Ф6 (коли утримання статики стане болем); одна велика зміна за раз, перевірки функціональні.
- Стабільні `@id`: Person `/pro-mene/#author`, ProfessionalService `/#business` (не змінюється при переїзді вузла);
  `@id` клієнта Ладомир — не чіпати.
- 28 CTA на `/#chat-section` + JSON-LD breadcrumbs на якорі головної перепривʼязуються у Ф1 до зміни головної;
  3 «напишіть мені» в постах про Паркінсон ведуть на особистий контакт, не в продажний чат.
- Рубрика Паркінсон — «досвід пацієнта» (без рецензента): дисклеймер, джерела, дата перевірки, журнал виправлень; поза пейволом.
- Продаж контенту через mono (інвойси + підписаний webhook, нативні регулярні платежі) — Ф5, заблоковано до визначення
  легальної структури продавця.

**Інфраструктура, що з'явилась:** RSS (`build-feed.mjs`, `inject-rss.mjs`, `/feed.xml`); загартований бекенд коментарів
(two-tier rate-limit, scoped errors, `safeEqual`); `sendToChannel()` під майбутній Telegram-канал.

**Статуси документів** — у таблиці «Документація» в `CLAUDE.md`.

## Підписка: RSS, Telegram, email (2026-09-21, Ф4)
- **Три канали.** RSS: `/feed.xml` (усе) + фіди чотирьох розділів; склад кожного — з `HUB_MEMBERS`, до 30
  найновіших. Telegram-канал `t.me/parkinsandr` (бот-адмін, `TELEGRAM_CHANNEL_ID` у Vercel). Email через Resend
  із домену `send.parkinsandr.tech`.
- **Публікація одною командою:** `npm run announce -- <розділ/slug>` — фіди + пост у канал + лист підписникам
  розділу. Для email потрібні `DATABASE_URL`, `RESEND_API_KEY`, `EMAIL_HASH_SECRET` у локальному `.env`.
- **Дані.** `subscribers` (адреса, обрані розділи, доказ згоди, хеші токенів) і `announcement_deliveries`
  (стан по кожному отримувачу). Токен відписки — детермінований HMAC від `email_hash`, у базі лише sha256;
  токен підтвердження одноразовий, діє 7 днів.
- **Строки зберігання** (збігаються з `/privacy/`): непідтверджені — 7 днів, хеш IP — 30 днів, адреса після
  відписки — 30 днів; далі лишаються тільки незворотні хеші й дати. Чистить крон `subscribers-retention` о 3:30.
- **Свідомі рішення:** `/api/unsubscribe` без CSRF-перевірки (one-click RFC 8058 приходить без `Origin`);
  відповідь `/api/subscribe` однакова для нової й уже підписаної адреси, але на відмову Resend — 503, а не
  «перевірте пошту»; підтверджує лише POST, бо поштові сканери переходять за GET-посиланнями.
- **Відоме обмеження:** часовий оракул на `/api/subscribe` (нова адреса = запис у БД + виклик Resend, уже
  підписана = миттєва відповідь). Прибирається чергою-outbox; не зроблено.
