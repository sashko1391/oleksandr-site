...
## 2026-07-14 → 2026-09-18 — аудит коментарів, нові пости, RSS, план переробки в особистий сайт

### Аудит і загартування коментарів (`0c7343a`, 2026-07-14)
- Видалено осиротілий `api/send-chat.js` + шляхи його воскресіння в `deploy.sh`/`update.sh` (чат і ліди — Cloudflare Worker).
- Two-tier rate-limit (pre-limit до Turnstile + authoritative bump після), scoped DB-помилки (`config_*` 500 / `db_*` 503),
  `safeEqual` для секретів (fail-closed на відсутній env), timeout і обробка помилок Telegram, прибрано flood circuit-breaker.
- Тести 16 → 57; живий smoke на проді 11/11. План: `doc/AUDIT_FIXES_PLAN.md` (4 раунди рев'ю).

### Контент (інші сесії)
- `4411987` «Ранній паркінсонізм простими словами» (YMYL-гайд) · `8ccc2af` «Говерла» з відео · `cb9457a` «Що робити вже зараз,
  якщо у вас хвороба Паркінсона» · `ffc033e` «Експеримент над собою» · `a40c6bd` оповідання «Відділ втраченого часу» ·
  `5de5580` кліп «Поверни мені час» · `de64049` SEO-кейс «SEO без реклами» · `2f5a492` журнал: одна хронологічна стрічка
  замість жанрових блоків.

### RSS — Фаза 0 плану підписки (`97303d1`, запушено 2026-09-18)
- `scripts/build-feed.mjs` (JSON-LD парситься як JSON, UTC RFC-822, XML-escape URL, sanitize control-chars, typed enclosures),
  `scripts/inject-rss.mjs` (ідемпотентно, quote-agnostic), `public/feed.xml`, `sendToChannel()` у `lib/telegram.js`.
- 3 раунди codex-рев'ю; тести 95/95. `/feed.xml` у проді з правильним Content-Type і кешем.

### Переробка в особистий сайт (рішення 2026-09-18)
- Підписку (`doc/SUBSCRIPTION_PLAN.md`) поставлено на паузу під нову IA.
- `doc/PERSONAL_SITE_PLAN.md` — позиція Claude + незалежна оцінка Codex, зведені в один план (Ф0–Ф7).
- Baseline Ф0 знято (сирі дані — gitignored `doc/baseline/`); модель ризиків переглянуто: SEO-ризик переробки низький,
  головна проблема — індексація журналу → трек «Індексація» (пріоритет №1).
- `doc/SERVICES_HUB_PLAN.md` — Ф1 v2 після рев'ю Codex (9 findings).
- Документацію й пам'ять приведено у відповідність: `CLAUDE.md` і `README.md` переписано, статуси на історичних планах.

### Lessons
- Інвентар JSON-LD будувати за повним URL/доменом, а не за суфіксом: шаблон `/#business` зачепив `@id` клієнта
  `ladomyr.kiev.ua/#business` — механічна заміна зламала б чужу сутність.
- `@id` сутностей — стабільні; переносити можна вузол і `url`, не ідентифікатор.
- GA4 «Короткий огляд звітів» не містить розбивки подій — для заявок потрібен звіт «Події».
- GSC Coverage-експорт без drill-down не дає переліку URL — непроіндексовані виводяться зі sitemap проти Performance (0 показів).
- Репо публічне: сирі метрики, фінансове, юридичне й особисте — лише в gitignored файлах і приватній пам'яті.

### AGENTS.md + policy-тести (2026-09-18)
- `AGENTS.md` — єдине джерело правил для всіх AI-агентів (Claude Code, Codex): кожне правило позначене
  `[enforced: …]` або `[advisory]`. `CLAUDE.md` став мостом, що імпортує `AGENTS.md` (`@AGENTS.md`) + Claude-специфічне.
- `tests/policy.test.js` (8 тестів) стереже статично перевірювані правила: немає font-preload; стабільні `@id`
  (без legacy `/#author`, `#business` — лише наш або клієнтський Ладомир); sitemap = рівно indexable-сторінки без дублів;
  RSS-лінк рівно один у `<head>` кожної сторінки. Мутаційна перевірка: навмисні порушення ловляться. Тести 95 → 103.

## 2026-09-19 → 2026-09-21 — Ф1, Ф1.5, Ф2, Ф3, трек індексації, Ф4 RSS по розділах

### Ф1 — комерційний хаб `/services/` (`269581e`, `b11021f`, `a7aac2b`, 2026-09-19)
- Якорі головної перепривʼязано на `/services/` скриптом `scripts/repoint-anchors.mjs` (план → перевірка
  лічильниками й семантичним diff JSON-LD → запис); фаза валідатора `f1-pre` → `f1-done`.
- Форма ховається після надісланої заявки; запит із таймаутом замість вічного «Надсилаю…».

### Ф1.5 — чесність форм і фактів (`e30ec8d` … `570ccd8`, 2026-09-20)
- Лід-форми показують успіх **лише** після того, як воркер підтвердив прийом; стани delivered/queued/lost,
  черга повторів у localStorage за id заявки. Модуль `public/js/lead-form.v1.js`, smoke 41 перевірка.
- Одна модель цін (джерело правди — `/pricing/`), одне формулювання відгуку з названим автором,
  кожна цифра на комерційній сторінці — з кейсу, дослідження або власного вимірювання з датою.
- Аудит «твердження → джерело» по 5 постах рубрики Паркінсон (`doc/PARKINSON_CLAIM_AUDIT.md`).
- Нові тести: `lead-forms`, `prices`, `testimonials`, `claims`, `faq-schema`, `images`.

### Ф2 — хаби розділів (`a46a0bb` … `dd13599`, 2026-09-21)
- `/parkinson/`, `/code/`, `/creative/` + архів `/blog/`; маніфест `HUB_MEMBERS` у `scripts/link-policy.mjs`
  як дані, покриття й повнота яких перевіряються незалежно від нього.
- Рамка рубрики («досвід пацієнта, не медична порада», дата звірки джерел, журнал виправлень, кризові контакти)
  на 5 постах + сторінка редакційної політики; рецензента-невролога немає — свідоме відхилення, назване вголос.
- Breadcrumbs ведуть на хаби, не на якорі головної → фаза `f2-done`; переробка головної нічого не ламає.

### Ф3 — особистий сайт (`1a69955`, `abafe8e`, `26ba573`, `8556717`, 2026-09-21)
- Розділ перейменовано на «Журнал»; наскрізне меню (`scripts/inject-nav.mjs`, ідемпотентне, розділ визначається
  з breadcrumbs JSON-LD); головна стала особистою — інтро, по три записи з чотирьох розділів, один блок про роботу.
- Скриптовий бот і єдиний вузол `ProfessionalService` переїхали на `/services/` (`@id` не змінився — правило 3).

### Рев'ю Codex (два раунди, `cca76f6`, `135f21f`)
- Підтверджені й виправлені: гонка черги бота (smoke-регресія з 10-секундним вікном), помилкові медичні цифри
  (дози MDS, DBS, агоністи), обіцянка «джерело під кожним реченням», дубль `ProfessionalService`,
  стеля в `priceRange`, неправдиве «три найновіші» на головній, відстежувана комерційна пропозиція в репо.
- Хибні findings відхилено з аргументом (напр. `lastmod` у sitemap без `dateModified` — поведінка навмисна).

### Трек індексації (`2d22850`, `10f4ccf`, 2026-09-21)
- Діагноз зі свіжого GSC: проблема не в позиціях, а в індексації — велика частина сторінок не проіндексована,
  найбільша група «Виявлено – наразі не проіндексовано» (цифри — gitignored `doc/baseline/`, правило 9).
- Причини, які можна було прибрати: мало внутрішніх посилань поза меню й дублі URL — сторінки віддавалися
  200 і без слеша, і на `/index.html`. `vercel.json`: `trailingSlash` + `cleanUrls` → 308 на канонічну форму.
- `robots.txt` переписано під актуальні імена AI-агентів; жодного Disallow. План — `doc/INDEXING_PLAN.md`,
  повторний вимір через 2–4 тижні.

### Ф4 — RSS по розділах (2026-09-21)
- `/parkinson/feed.xml`, `/code/feed.xml`, `/creative/feed.xml`, `/journal/feed.xml` поверх загального `/feed.xml`.
  Склад фіда — до 30 найновіших постів розділу за `HUB_MEMBERS`, тобто те саме, що показує хаб.
- `collect()` більше не ріже до 30 — ліміт застосовується на кожен фід окремо; `build(items, feed)` рендерить
  канал переданого фіда; кожен item знає файл-джерело.
- Autodiscovery фіда розділу — лише на його хабі; видиме посилання — у блоці «Поруч» (і окремим блоком у журналі).
- Тести 329 → 341; мутаційна перевірка: 6 мутантів, 6 убито. Перший мутант («фід бере лише `primary`, без `also`»)
  вижив, бо тест читав тільки записаний файл — тепер тест звіряє з маніфестом і файл, і те, що згенерував би скрипт.

### Ф4, крок 2 — Telegram-канал (2026-09-21)
- Власник створив публічний `t.me/parkinsandr`, додав бота адміністратором, `TELEGRAM_CHANNEL_ID` — у Vercel env.
- `scripts/announce.mjs` — анонс поста: регенерація фідів + пост у канал через наявний `sendToChannel()`.
  Голий canonical у тексті (щоб Telegram намалював OG-картку), жирний заголовок, тизер, один хештег.
- Хештег — із `HUB_MEMBERS.primary`, не з `articleSection`: секція є лише в 12 із 28 постів і суперечить сама
  собі («Есеї» / «Проза» / «Журнал» / «Блог» вживаються як синоніми).
- Секрети — лише з gitignored `.env`; що вже анонсовано — gitignored `.announce-state.json`, тож повторний
  прогін нічого не дублює.
- Видимі посилання на канал — на чотирьох хабах поруч із RSS; URL винесено в константу `TELEGRAM_CHANNEL`,
  тест ловить появу другої адреси (на сайті свідомо дві різні: канал і особистий контакт на комерційних).
- Тести 354 → 369; мутаційна перевірка `announce.mjs`: 6 мутантів — 6 убито.

### Рев'ю Codex по Ф4 (без High; 3 Medium, 5 Low — усі прийняті)
- **Ліміт 30 суперечив обіцянці «рівно те, що показує хаб».** Модель зафіксовано: «до 30 найновіших у кожному
  фіді»; тест рахує очікуване з маніфесту з тим самим відсіканням. Заодно знайдено непослідовний comparator:
  при однакових датах він повертав `-1` в обидва боки — а три пости мають дату 2026-07-13, тож відсікання було
  недетермінованим. Тепер tie-break за файлом-джерелом.
- **Regex бачив розмітку в коментарях і `<script>` як справжні посилання.** Додано `maskInert()` — інертні
  ділянки забілюються зі збереженням офсетів, тож індекси досі вказують у справжній HTML. Використано і в
  інжекторі, і в `tests/policy.test.js` (той ще й вимагає справжній `rel=alternate` + RSS MIME, а не будь-який href).
- **«exactly» не було enforced:** дубль `<item>`, зниклий `<pubDate>`, підмінений `<title>` при тому самому GUID
  чи зламаний XML проходили повз тести. Додано звірку записаного файла з `build(itemsFor(feed, collect()), feed)`
  з точністю до `lastBuildDate` + перевірку обовʼязкових елементів у кожному item.
- **Дрібне:** `attrOf()` читає href у будь-яких лапках; `xmlSafe()` тепер відповідає Char-продукції XML 1.0
  (U+FFFE/U+FFFF і непарні сурогати); `image/title` = title каналу, а сам логотип — `images/rss-logo.png`
  144×144 (рендер favicon) замість OG-картинки 1200×630, із заявленими `width`/`height`: без них рідери
  беруть дефолт 88×31 (RSS 2.0).
- **Autodiscovery:** фід розділу тепер іде **першим** і стоїть не лише на хабі, а й на постах розділу —
  клієнт, що розуміє один фід, підпишеться на тематичний. Рішення винесено в чисту `tagsFor()`, бо мутація
  всередині `run()` тестами не ловилась.
- Мутаційна перевірка після правок: 20 мутантів — 20 убито. Тести 341 → 353.

### Lessons
- Тест, який перевіряє лише **записаний** артефакт, не ловить мутацію в генераторі: треба звіряти і те,
  що на диску, і те, що згенерувалося б зараз. Якщо рішення живе в `run()`, тест його не дістане —
  виносити в чисту функцію.
- Comparator, що повертає `-1` для рівних елементів, ламає детермінізм сортування там, де є ліміт.
- Дубль URL створює сам хостинг: `cleanUrls`/`trailingSlash` без явної канонічної форми дають два 200 на сторінку.
- Playwright не бачить текст усередині `<noscript>` — перевіряти через DOM, а не текстовий двигун.

## 2026-09-21/22 — Ф4 доведено до кінця: email-розсилка, два інциденти, пост про пісню

### Email-підписка (`d172e62`, `6cca83d`, `12dfbcd`, `fad859b`)
- Resend верифікував `send.parkinsandr.tech`; DNS: DKIM + два CNAME, корінь `v=spf1 -all` і `_dmarc p=reject`,
  піддомен `_dmarc.send` спершу `p=none`, після успішного тесту — `p=quarantine`.
- **По ходу знайдено два `v=spf1` на корені.** Це PermError: SPF не працює взагалі, а не «стає суворішим».
  Прибрано запис хостингу (скриньки на домені немає).
- Таблиці `subscribers` і `announcement_deliveries` (`db/*.sql`) застосовані; перевірено живими вставками, що
  спрацьовують усі CHECK-обмеження. `citext` перенесено з `public` у `extensions` за вказівкою лінтера Supabase.
- `/api/subscribe` + `confirm` + `unsubscribe` + крон-очистка + віджет + секція «Розсилка» в `/privacy/`.

### Розсилка (`1f123e1`)
- `announce.mjs` тепер робить три канали. Стан доставки — **по кожному отримувачу**, бо груба позначка
  «пост розіслано» при падінні на 50-му листі лишила б решту без листа назавжди.
- Рядок займається до виклику Resend, ключ ідемпотентності детермінований; програний claim-race означає
  «не слати», а не «слати ще раз». 90 листів за прогін і 600 мс паузи — під безкоштовний ліміт.

### Рев'ю Codex (13 findings, усі опрацьовані)
- **High, найдорожче:** обидва cron-шляхи не мали кінцевого слеша, а `trailingSlash: true` → 308. Vercel Cron за
  редиректом не йде, тож **очистка жодного разу не виконувалась**, хоча `/privacy/` обіцяв строки.
- **High:** токен відписки був невідновлюваний — сирий викидався одразу після хешування, тож розсилка не змогла б
  скласти посилання. Став детермінованим (HMAC від `email_hash`).
- **High:** immutable-файл `subscribe.v1.js` міняли двічі за тією самою адресою → бамп до `v2`.
- **Medium:** cooldown перевірявся в JS перед UPDATE (гонка) → став умовою самого UPDATE; ліміти рахуються по
  скриньці, а не по написанню адреси (`victim+7@gmail.com` = `victim@gmail.com`).
- Решта — строк дії посилання як умова підтвердження, збереження suppression-рядка, точні формулювання в
  `/privacy/`, віджет без зависань, строгий розбір прапорців у CLI, атомарний запис фідів і стану.

### Інцидент 1: зникло сховище Upstash KV
Обидві форми віддавали 503. Виявилося, що безкоштовна база Upstash зникла, а змінні у Vercel вказували на мертвий
стор (Vercel API: `404 Integration Resource not found`). Оскільки лічильники недоступні, код працює fail-closed —
це правильно, але **коментарі не приймалися з 11 серпня**, і дізналися ми про це випадково, через підписку.
Створено новий стор, старі змінні видалено (нове підключення не могло їх перезаписати), редеплой.

### Інцидент 2: зміна пароля бази без оновлення Vercel
Після ротації `EMAIL_HASH_SECRET` і пароля бази функції почали висіти: `/api/comments/` → 504, статус 0 у логах.
`DATABASE_URL` у Vercel лишився зі старим паролем. Лікується оновленням змінної + редеплоєм.

### Живий тест ланцюжка (пройдено)
Підписка → лист **у вхідних**, не в спамі → підтвердження за 60 с → анонс поста в канал і листом (`sent` +
`resend_id` у базі) → **кнопка «Відписатися» в інтерфейсі Gmail** працює (POST без `Origin` — саме те, заради чого
на цьому ендпоінті немає CSRF-перевірки) → повторна підписка. Три натискання «підписатися» дали один лист.

### Контент
Пост про пісню «Ми ж тільки на каву!» (`4bba31a`): Article + MusicRecording, плеєр-фасад, інструменти названі
(Suno — музика, ChatGPT — обкладинка), ідея дружини. Текст пісні не публікуємо — рішення власника.
На хабі «Творчість» виправлено твердження «усе це я пишу сам: ідея, текст, музика» — після цієї пісні воно
перестало бути правдою.

### Lessons
- **Зовнішня залежність може тихо померти, і фейл-клоуз це приховає.** Форма не «зламалася» — вона коректно
  відмовляла місяць. Потрібен моніторинг: `POST` із невалідною капчою має давати 400, а не 503.
- **Секрет із похідними даними не ротується безболісно.** `EMAIL_HASH_SECRET` визначає хеш адреси й токени
  відписки; замінювати його можна дешево лише поки підписників одиниці.
- **Зміна пароля бази — це два місця:** Supabase і `DATABASE_URL` у Vercel, плюс редеплой.
- **`trailingSlash` стосується і `/api/*`, і cron-шляхів.** Один відсутній слеш зробив очистку неіснуючою.
- Прев'ю, яке показує лише перший із двох каналів, — не прев'ю (`--dry` виходив до email-частини).
