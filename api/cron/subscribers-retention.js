...
export default async function handler(req, res) {
  ...
  try {
    const sql = getSql();
    // coalesce(confirm_sent_at, created_at): someone who re-subscribed after unsubscribing has an old
    // created_at, and must get the full 7 days from the letter they were actually sent.
    ...
    const erased = await sql`
      UPDATE subscribers SET email = NULL
      WHERE status = 'unsubscribed' AND email IS NOT NULL
        AND unsubscribed_at < now() - interval '30 days'
      RETURNING id`;
    const result = { unconfirmed: unconfirmed.length, ips: ips.length, erased: erased.length };
