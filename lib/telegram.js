...
/**
 * Post a public announcement to the broadcast channel (TELEGRAM_CHANNEL_ID).
 * Unlike sendMessage (private moderation chat) this KEEPS the web preview so the
 * post shows its OG card. Used by scripts/announce.mjs on publish.
 * @param {string} text HTML
 */
export function sendToChannel(text) {
  const chatId = process.env.TELEGRAM_CHANNEL_ID;
  if (!chatId) throw new Error('TELEGRAM_CHANNEL_ID is not configured');
  return call('sendMessage', {
    chat_id: chatId,
    text,
    parse_mode: 'HTML',
